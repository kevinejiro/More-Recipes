...
const verifyUserSession = (req, res, next) => {
  ...
  // if no token found -->
  if (!token) {
    return res.status(401).json({ status: 'fail', message: 'An authorization token is required!' });
  } // Check if token matches the one provided at login or signup
  jwt.verify(token, process.env.SECRET_KEY, (err, decoded) => {
    if (err) {
      if (err.name === 'TokenExpiredError') { return res.status(401).json({ status: 'pass', message: 'Session has expired, Please sign-in again' }); }
